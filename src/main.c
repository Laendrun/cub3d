...
/*   Updated: 2023/03/03 22:58:19 by saeby            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int ac, char **av)
{
	...
	if (ac != 2)
		return (puterr("Wrong number of arguments. Usage ./cub3d <map.cub>", 0));
	env.map.path = av[1];
	if (init_cub3d(&env))
		return (1);
...
int	render(t_env *env)
{
	...
	while (y < env->map2d.height)
	{
		...
		{
			//printf("%d", env->map2d.px[x + y * env->map2d.width]);
			put_px(env, (t_v2){x, y}, env->map2d.px[x + y * env->map2d.width], 2);
			x++;
		}
		y++;
	}
	raycasting(env);
	// printf("current coord, X: %f, Y: %f\n", env->player.pos.x, env->player.pos.y);
